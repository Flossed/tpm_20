...
try {
    ...
    try {
        ...
        try {
            ...
            Write-Host "Using Microsoft Software Key Storage Provider (software fallback)" -ForegroundColor Yellow
        } catch {
            # Let system choose provider
            $providerName = "System Default"
            Write-Host "Using system default provider" -ForegroundColor Yellow
        }
    }
    ...
    if ($key) {
        ...
        try {
            ...
            Write-Host "Warning: Key created but cannot be reopened" -ForegroundColor Yellow
        }

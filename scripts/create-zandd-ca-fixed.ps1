# ZANDD Certificate Authority - Create Root CA (Fixed Version)
# Creates a local CA for issuing X.509 certificates with TPM entropy
...
Write-Host "=== ZANDD Certificate Authority Setup (Fixed) ===" -ForegroundColor Cyan
...
try {
    # Step 1: Generate CA Root Key with TPM entropy
    $caKey = $null
    $keyCreationTime = Measure-Command {
        if ($UseTPM) {
            Write-Host "  Using TPM for hardware entropy generation..." -ForegroundColor Yellow
            
            # Method 1: Use RNGCng for hardware entropy from TPM
            try {
                $rng = [System.Security.Cryptography.RNGCng]::Create()
                $entropyBytes = [byte[]]::new(32)  # 256 bits of entropy
                $rng.GetBytes($entropyBytes)
                $rng.Dispose()
                
                Write-Host "    Hardware entropy generated: 32 bytes" -ForegroundColor Gray
                
                # Combine with timestamp for uniqueness
                $combinedEntropy = $entropyBytes + [System.Text.Encoding]::UTF8.GetBytes("ZANDD-CA-$(Get-Date -Format 'yyyyMMddHHmmssfff')")
                $keyEntropy = [System.Security.Cryptography.SHA256]::HashData($combinedEntropy)
                
                # Create software ECDSA key (reliable)
                $caKey = [System.Security.Cryptography.ECDsa]::Create([System.Security.Cryptography.ECCurve+NamedCurves]::nistP256)
                
                Write-Host "  ✓ Software key created with TPM hardware entropy" -ForegroundColor Green
            }
            catch {
                Write-Host "    TPM entropy failed, falling back to software: $($_.Exception.Message)" -ForegroundColor Yellow
                $UseTPM = $false
            }
        }
        ...
        Write-Host "  ✓ Private key exported and saved" -ForegroundColor Green
    }
...
"@
    ...
    try {
        ...
        Write-Host "  ⚠ Could not install in Windows trust store: $($_.Exception.Message)" -ForegroundColor Yellow
    }
    ...
    $caConfig = @{
        ...
        KeyBackend = if ($UseTPM) { "TPM Hardware Entropy + Software" } else { "Software" }
        CAPath = $CAPath
    }
...
"@
    ...
    Write-Host "  Key Type: $(if ($UseTPM) { 'TPM Hardware Entropy + Software' } else { 'Software' })" -ForegroundColor White
    Write-Host ""
